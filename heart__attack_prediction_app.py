...
with col2:
    ...
    with st.form("Diabetes Predictor App"):
        ...
        trtbps = st.sidebar.slider("Resting Blood Pressure", 0, 200)
        chol = st.sidebar.slider("Cholesterol Level", 100, 600)
        fbs = st.sidebar.slider("Fasting Blood Sugar (1 = True, 0 = False) ", 0, 1)
        rest_ecg = st.sidebar.slider("Resting Electrocardiographic Results (0 = Normal, 1 = ST-T wave normality, 2 = Left ventricular hypertrophy) ", 0, 1, 2)
        thalach = st.sidebar.slider("Maximum Heart Rate", 0, 220)
        exng = st.sidebar.slider("Exercise Induced Angina (1 = Yes, 0 = No)", 0, 1)
        oldpeak = st.sidebar.slider("ST Depression Induced by Exercise Relative to rest",0.0,7.0,0.8,0.1)
        slp = st.sidebar.slider('Slope', 0, 2)
        caa = st.sidebar.slider('Number of Major Vessels', 0, 3)
        thall = st.sidebar.slider('Thalium Stress Test result', 0, 2, 3)
